...
?>
<?php $total = buddypress_search_get_total_lessons_count( get_the_ID() ); ?>
<div class="bp-search-ajax-item bp-search-ajax-item_sfwd-courses">
	<a href="<?php echo esc_url( add_query_arg( array( 'no_frame' => '1' ), get_permalink() ) ); ?>">
		...
		<div class="item">
			...
			<div class="item-desc">
			<?php
			/* translators: %s: the number of total lesson count */
			$get_lesson_total_count = sprintf( _n( '%d lesson', '%d lessons', $total, 'buddypress-search' ), $total );
			echo esc_html( $get_lesson_total_count );
			?>
			</div>
