...
if ( ! class_exists( 'Wbcom_Admin_Settings' ) ) {
	...
	class Wbcom_Admin_Settings {
		...
		public function wbcom_enqueue_admin_scripts() {
			...
			if ( ! wp_script_is( 'wbcom_admin_setting_js', 'enqueued' ) ) {
				...
				wp_register_script(
					...
					$src       = BP_SEARCH_PLUGIN_URL . 'admin/wbcom/assets/js/wbcom-admin-setting.js',
					$deps      = array( 'jquery' ),
					$ver       = time(),
					$in_footer = true
				);
				wp_localize_script(
					...
					array(
						...
						'activate_text'   => esc_html__( 'Activate', 'buddypress-search' ),
						'deactivate_text' => esc_html__( 'Deactivate', 'buddypress-search' ),
			...
			if ( ! wp_style_is( 'wbcom-admin-setting-css', 'enqueued' ) ) {
				wp_enqueue_style( 'wbcom-admin-setting-css', BP_SEARCH_PLUGIN_URL . 'admin/wbcom/assets/css/wbcom-admin-setting.css' );
			}
		...
		public function wbcom_admin_additional_pages() {
			add_submenu_page(
				'wbcomplugins',
				esc_html__( 'Our Plugins', 'buddypress-search' ),
				esc_html__( 'Our Plugins', 'buddypress-search' ),
				'manage_options',
				'wbcom-plugins-page',
			...
			add_submenu_page(
				'wbcomplugins',
				esc_html__( 'Our Themes', 'buddypress-search' ),
				esc_html__( 'Our Themes', 'buddypress-search' ),
				'manage_options',
				'wbcom-themes-page',
			...
			add_submenu_page(
				'wbcomplugins',
				esc_html__( 'Support', 'buddypress-search' ),
				esc_html__( 'Support', 'buddypress-search' ),
				'manage_options',
				'wbcom-support-page',
					...
					<ul>
						<li class="wb_admin_nav_item <?php echo esc_attr( $settings_active ); ?>">
							<a href="<?php echo esc_url( get_admin_url() ) . 'admin.php?page=wbcomplugins'; ?>" id="wb_admin_nav_trigger_settings">
								...
								<h4><?php esc_html_e( 'Settings', 'buddypress-search' ); ?></h4>
							</a>
						</li>
						<li class="wb_admin_nav_item <?php echo esc_attr( $plugin_active ); ?>">
							<a href="<?php echo esc_url( get_admin_url() ) . 'admin.php?page=wbcom-plugins-page'; ?>" id="wb_admin_nav_trigger_extensions">
								<i class="fa fa-th"></i>
								<h4><?php esc_html_e( 'Themes & Extension', 'buddypress-search' ); ?></h4>
							</a>
						</li>
						...
						<li class="wb_admin_nav_item <?php echo esc_attr( $support_active ); ?>">
							<a href="<?php echo esc_url( get_admin_url() ) . 'admin.php?page=wbcom-support-page'; ?>" id="wb_admin_nav_trigger_support">
								...
								<h4><?php esc_html_e( 'Support', 'buddypress-search' ); ?></h4>
							</a>
						</li>
